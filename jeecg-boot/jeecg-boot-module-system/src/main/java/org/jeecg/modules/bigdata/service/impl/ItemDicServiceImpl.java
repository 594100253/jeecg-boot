...
public class ItemDicServiceImpl extends ServiceImpl<ItemDicMapper, ItemDic> implements IItemDicService {
	...
	@Override
	public List<Map> getEventItemIds(Integer eventId) {
		String sql=" select " +
...
//			"GROUP BY is_hierarchy";
	...
	@Override
	public Map getConcatColumn(Map map) {
		String sql="select is_hierarchy\n" +
			"from bu_item_dic id \n" +
			"where is_hierarchy !=0 and id in ("+ map.get("itemIds") +")\n" +
			"GROUP BY is_hierarchy";
		List<Map> list  = eventDicMapper.superSelect(sql);
		...
		sqlHierarchyItemSb.append(
			"\t\t\t(\n" +
				"\t\t\t\tselect id.item_min_width width,id.item_name tableTitle,id.db_column_name tableName,id.db_column_name groupName,id.db_column_sql selectName,id.sort_no,id.id\n" +
				"\t\t\t\tFROM bu_event_dic ed\n" +
				"\t\t\t\tLEFT JOIN bu_event_item ei ON ei.event_id = ed.id\n" +
				"\t\t\t\tLEFT JOIN bu_item_dic id ON id.id = ei.item_id\n" +
				"\t\t\t\tWHERE\n" +
				"\t\t\t\t\ted.id = '"+map.get("eventId")+"'\n" +
				"\t\t\t\tAND is_hierarchy = 0\n" +
				"\t\t\t\tAND id.id IN ("+ map.get("itemIds") +")\n" +
				"\t\t\t)\n"
		);
		// 层级维度 sql
		for (int i = 0; i < list.size() ; i++) {
			sqlHierarchyItemSb.append(
				"\t\t\tunion\n" +
					"\t\t\t( \n" +
					"\t\t\t\tselect item_min_width width,item_name tableTitle,db_column_name tableName,db_column_sql groupName,db_column_sql selectName,sort_no,id\n" +
					"\t\t\t\tfrom bu_item_dic where is_hierarchy ='"+list.get(i).get("is_hierarchy")+"'\n" +
					"\t\t\t\tand id in ("+ map.get("itemIds") +")\n" +
					"\t\t\t\torder by id desc LIMIT 1\n" +
					"\t\t\t)\n"
			);
		}
		sqlHierarchyItemSb.insert(0,
			"\t\tselect width,tableTitle,tableName,groupName,selectName\n" +
				"\t\tfrom(\n"
		);
		...
		sqlHierarchyItemSb.insert(0,
			"\tselect\n" +
				...
				"\t\tGROUP_CONCAT(item.tableTitle) tableTitle,GROUP_CONCAT(item.tableName) tableName,\n" +
				"\t\tGROUP_CONCAT(item.selectName) selectName,GROUP_CONCAT(item.groupName) groupName,null fromName\n" +
				"\tfrom (\n");
		...
		sqlHierarchyItemSb.insert(0,
			"select " +
				...
				"\tGROUP_CONCAT(a.tableTitle) tableTitle,GROUP_CONCAT(a.tableName) tableName,\n" +
		...
		sqlHierarchyItemSb.append(
			"\tunion all\n" +
				...
				"\t\t\twhere ed.id='"+map.get("eventId")+"'\n" +
				"\t\t\tand td.id in ("+map.get("targetIds")+")\n" +
